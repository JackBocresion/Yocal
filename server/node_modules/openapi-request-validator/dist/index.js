...
var OpenAPIRequestValidator = /** @class */ (function () {
    function OpenAPIRequestValidator(args) {
        ...
        var v = new Ajv(__assign({ useDefaults: true, allErrors: true, unknownFormats: 'ignore', missingRefs: 'fail', 
        ...
        if (args.componentSchemas) {
            ...
            Object.keys(args.componentSchemas).forEach(function (id) {
                ...
                _this.addSchemaProperties(v, args.componentSchemas[id], "#/components/schemas/" + id);
            });
        }
                ...
                switch (attr) {
                    ...
                    case 'anyOf':
                        for (var i = 0; i < schema[attr].length; i++) {
                            this.addSchemaProperties(v, schema[attr][i], prefix + "/" + attr + "/" + i);
                        }
                        return;
                    case 'items':
                    ...
                    case 'properties':
                        for (var propertyId in schema[attr]) {
                            if (schema[attr].hasOwnProperty(propertyId)) {
                                var schemaId = prefix + "/" + attr + "/" + propertyId;
